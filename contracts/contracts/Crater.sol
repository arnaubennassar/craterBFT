...
import "./interfaces/ICraterErrors.sol";
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import "@openzeppelin/contracts/utils/cryptography/ECDSA.sol";

contract Crater is ICraterErrors {
        /**
    ...
    function sendComet(Comet calldata comet, bytes calldata signaturesAndAddrs) external {
        if (comet.nonce != nonce + 1) {
            revert InvalidNonce();
        }
        if (comet.blockDeadline > block.number) {
            revert BlockDeadlineExceeded();
        }
        // Hash the msg
        ...
        this.verifySignatures(msgHash, signaturesAndAddrs);
        craters[nonce] = comet.message;
        nonce++;
    }
}